...
import { Trash, Clock, Heart, Search, Film, Tv, Calendar, X, Filter, ChevronDown, User } from "lucide-react";
...
const SearchHistoryPage = () => {
	...
	const filteredItems = filter === "all"
		? searchHistory
		: filter === "person"
			? searchHistory.filter(item => item.searchType === "person")
			: filter === "movie"
				? searchHistory.filter(item => item.searchType === "movie")
				: searchHistory.filter(item => item.searchType === "tv");
						...
						<div className="relative">
							...
							{showFilterMenu && (
								<div className="absolute right-0 mt-1 bg-gray-800 rounded-md shadow-lg border border-gray-700 z-10 w-48">
									...
									<button
										className={`w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-2 ${filter === "person" ? "bg-gray-700" : ""}`}
										onClick={() => { setFilter("person"); setShowFilterMenu(false); }}
									>
										<User size={14} />
										<span>People</span>
									</button>
						...
						{filteredItems.map((entry) => (
							...
							>
								...
								<div className="flex-grow p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
									...
									<div className="flex items-center gap-3">
										<span
											className={`py-1 px-3 rounded-full text-xs font-medium flex items-center gap-1 ${entry.searchType === "movie"
													? "bg-red-600"
													: "bg-blue-600"
												}`}
										>
